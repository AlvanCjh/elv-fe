...
import { useMaintenanceReports, MaintenanceReport } from '../../scheduling/scheduleApi';
import MaintenanceReportDialog from './MaintenanceReportDialog';
import { motion, AnimatePresence } from 'framer-motion';
import { API_BASE_URL } from '@/utils/api';
import useUser from '@auth/useUser';

function MaintenanceReportsWidget() {
    const [startDate, setStartDate] = useState('');
    const [endDate, setEndDate] = useState('');
    const { data: reports, isLoading } = useMaintenanceReports(startDate, endDate);
    ...
    return (
        <Box className="flex flex-col gap-8 p-6 lg:p-8">
            ...
            <Box className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <Box>
                    <Typography variant="h4" className="font-black text-gray-800 dark:text-gray-100 flex items-center gap-3">
                        <FuseSvgIcon size={32} className="text-green-500">heroicons-outline:wrench-screwdriver</FuseSvgIcon>
                        Maintenance Dashboard
                    </Typography>
                    <Typography variant="body1" className="text-gray-500 font-medium mt-1">
                        Manage equipment maintenance logs and systemic task tracking.
                    </Typography>
                </Box>
                {isSupervisor && (
                    <Button 
            ...
            <Box className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {[
                    { label: 'Total Logs', value: stats.total, color: 'blue', icon: 'heroicons-outline:document-text' },
                    { label: 'Pending', value: stats.pending, color: 'amber', icon: 'heroicons-outline:clock' },
                    { label: 'Fixed/Ready', value: stats.completed, color: 'green', icon: 'heroicons-outline:check-circle' }
                ].map((stat) => (
                        ...
                        return (
                            ...
                            >
                                <Paper className="relative overflow-hidden p-6 rounded-[40px] shadow-sm hover:shadow-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-800 transition-all group flex flex-col h-full ring-blue-500/0 hover:ring-2">
                                    ...
                                    {report.image_path && (
                                        <Box className="mb-4 rounded-2xl overflow-hidden h-32 w-full">
                                            <img 
                                                src={`${API_BASE_URL}/storage/${report.image_path}`} 
                                                alt={report.title}
                                    ...
                                    <Typography variant="body2" className="text-gray-500 dark:text-gray-400 mb-8 line-clamp-3 leading-relaxed">
                                        {report.description || 'No detailed description available for this maintenance log.'}
                                    </Typography>

                                    <Box className="mt-auto pt-6 border-t border-gray-100 dark:border-gray-700/50 flex flex-col gap-4">
                                        <Box className="flex items-center justify-between">
                                            <Box className="flex flex-col">
                                                <Typography variant="caption" className="text-gray-400 font-bold uppercase tracking-tighter">Technician</Typography>
                                            ...
                                            <Box className="flex items-center gap-1.5 text-gray-400 bg-gray-50 dark:bg-gray-700/50 px-3 py-1.5 rounded-2xl">
                                                ...
                                                <Typography variant="caption" className="font-black">
                                                    {new Date(report.maintenance_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                                                </Typography>
                                            </Box>
                                        </Box>
                                    </Box>
            ...
            {filteredReports.length === 0 && (
                <Box className="flex flex-col items-center justify-center py-32 text-center animate-pulse">
                    <Avatar className="bg-gray-50 dark:bg-gray-800/50 w-32 h-32 mb-6 text-gray-200">
                        <FuseSvgIcon size={64}>heroicons-outline:wrench-screwdriver</FuseSvgIcon>
                    </Avatar>
                    <Typography variant="h5" className="font-black text-gray-300 uppercase tracking-tighter">No logs found</Typography>
                    <Typography variant="body1" className="text-gray-400 mt-2 max-w-sm">
                        Keep moving! Double check your search or filters to locate specific maintenance reports.
                    </Typography>
                </Box>
            )}
...
export default MaintenanceReportsWidget;
